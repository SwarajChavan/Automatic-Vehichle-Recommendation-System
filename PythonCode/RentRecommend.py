...
def getdata():
   ...
   SelectQuery = "SELECT user, vehicle,value FROM rentdataset order by id desc"
...
def getuserdata(userid):
   db = mysql.connector.connect(host="localhost",database="be_project",user="root",password="")
   SelectQuery = "SELECT user, vehicle,value FROM rentdataset where user='%s' order by id desc" %(userid)
   resp = pd.read_sql_query(SelectQuery, db)       
   # disconnect from server
   db.close()
   return resp;
