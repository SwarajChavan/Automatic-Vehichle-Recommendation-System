...
%>
		...
		if(rs.next()){
			if(rs.getString("purpose").equals("Sell")){
				PreparedStatement stmt=conn.prepareStatement("INSERT INTO `buydataset`(`user`, `vehicle`) VALUES (?,?)");
								...
								<div class="col-md-5">
									<div class="product-entry">
										<div class="product-img" style="background-image: url(GetImages?id=<%=rs.getString("id")%>);">
											<p class="tag"><span class="sale"><%=rs.getString("inspection")%></span></p>
										</div>
									</div>
								</div>
								<div class="col-md-7">
									<div class="desc">
										...
										<%
										if(rs.getString("purpose").equals("Rent")){
										%>
										<form action="ContactDealer" method="get"><p>Start Date<p>
                                    	<input type="date" id="date" name="sdate" class="form-control" required="required"><br>
                                    	<p>End Date<p>
                                    	<input type="date" id="edate" name="edate" class="form-control" required="required"><br>
                                    	<input type="hidden" name="id" value="<%=rs.getString("id")%>">
                                    	<input type="hidden" name="purpose" value="<%=rs.getString("purpose") %>"> 
                                    	<p><button type="submit" class="btn btn-primary btn-addtocart">Contact Dealer</button></p>
										</form>
										<%}else{ %>
										<form action="ContactDealer" method="get">
                                    	<input type="hidden" name="id" value="<%=rs.getString("id")%>">
                                    	<input type="hidden" name="purpose" value="<%=rs.getString("purpose") %>"> 
                                    	<p><button type="submit" class="btn btn-primary btn-addtocart">Contact Dealer</button></p>
										</form>
						...
						<div class="row">
							<div class="col-md-12 tabulation">
								<ul class="nav nav-tabs">
									...
									<li><a data-toggle="tab" href="#dealer">Dealer</a></li>
								</ul>
								<div class="tab-content">
						         ...
						         <%
						         PreparedStatement stmt=conn.prepareStatement("SELECT * FROM `dealer` where email=?");
						         stmt.setString(1, rs.getString("dealer"));
